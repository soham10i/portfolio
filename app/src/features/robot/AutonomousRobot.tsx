...
import MazeWorld, { type MazeKey, type SimState } from '@/features/robot/components/MazeWorld';
...
interface MazeMeta {
  ...
  cylinders: number;
  span: number;
  /** Recorded Webots run from the real simulator, in public/videos/. */
  video: string | null;
}

const MAZES: MazeMeta[] = [
  { key: 'maze1', label: 'Map 1', title: 'Open floor',        difficulty: 'Baseline', controller: 'mak_02_controller', walls: 38, pads: 1, cylinders: 2, span: 4.71, video: '/videos/maze1.mp4' },
  { key: 'maze2', label: 'Map 2', title: 'Corridors',         difficulty: 'Moderate', controller: 'mak_02_controller', walls: 68, pads: 3, cylinders: 2, span: 8.84, video: '/videos/maze2.mp4' },
  { key: 'maze3', label: 'Map 3', title: 'Loop closure',      difficulty: 'Moderate', controller: 'mak_04_controller', walls: 37, pads: 8, cylinders: 2, span: 6.57, video: '/videos/maze3.mp4' },
  ...
  { key: 'maze5', label: 'Map 5', title: 'Dynamic obstacles', difficulty: 'Hard',     controller: 'mak_02_controller', walls: 18, pads: 2, cylinders: 2, span: 5.55, video: '/videos/maze5.mp4' },
];
...
export default function AutonomousRobot() {
  ...
  /* Maze 4's exported world seals the robot's start pose inside a closed
     pocket, so the engine relocates the spawn to the nearest open cell. Say so
     rather than quietly running from somewhere the Webots world does not. */
  const moved = sim?.spawnMoved;
  const spawnNote = !moved
    ? null
    : 'sealed' in moved
      ? 'Exported spawn is enclosed — no open start found'
      : `Spawn relocated ${moved.by} m — exported start was sealed`;
    ...
    <div className="min-h-screen bg-bg text-fg">
      ...
      <main className="mx-auto max-w-[1180px] px-6 py-8">
        ...
        <section className="mt-6 overflow-hidden rounded-[18px] border border-line bg-surf">
          ...
          {/* Canvas / video */}
          <div className="relative aspect-[4/5] w-full bg-canvas sm:aspect-video">
            {view === 'video' ? (
              maze.video ? (
                <video
                  key={maze.video}
                  src={maze.video}
                  /* The poster is a real frame from the clip, so the panel is
                     never an empty black box while the video buffers — which
                     matters most on the first visit, and on mobile where
                     autoplay may not start at all. */
                  poster={maze.video.replace(/\.mp4$/, '.jpg')}
                  controls
                  autoPlay
                  muted
                  loop
                  playsInline
                  /* metadata, not auto: five clips on one page must not all
                     start downloading because the tab strip rendered. */
                  preload="metadata"
              ...
              <>
                ...
                {/* HUD */}
                {spawnNote && (
                  <div className="pointer-events-none absolute inset-x-3.5 bottom-16 flex justify-center">
                    <span className="glass-panel rounded-[9px] px-2.5 py-1.5 font-mono text-[9.5px] uppercase tracking-[0.12em] text-fg2">
                      {spawnNote}
                    </span>
                  </div>
                )}
