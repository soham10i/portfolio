...
import FactoryTwin from '@/features/factory-twin/components/FactoryTwin';
import {
  CYCLE_TOTAL,
  PHASE_SEGMENTS,
  STATIONS,
  phaseAt,
  type StationId,
} from '@/features/factory-twin/lib/twin';
import { PALETTES, applyPalette, readStoredPalette, type PaletteKey } from '@/shared/lib/palettes';
import { ACTUATOR_SPECS, ioState } from '@/features/factory-twin/lib/plcIo';
...
export default function FactoryTwin3D() {
  ...
  const io = ioState(phase);
  const onCount = io.filter((p) => p.on).length;
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-bg text-fg">
      ...
      <header className="flex min-h-[60px] shrink-0 flex-wrap items-center justify-between gap-2 border-b border-line px-4 py-2 sm:h-[60px] sm:flex-nowrap sm:px-[22px] sm:py-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="hidden font-mono text-[10.5px] uppercase tracking-[0.1em] text-fg3 sm:block">FISCHERTECHNIK 536634 · 24 V · 9 DI / 14 DO · WS://STF-GATEWAY</p>
          </div>
        </div>
      ...
      <div className="flex min-h-0 flex-1">
          ...
          {source === 'webots' ? (
            ...
            <div className="absolute inset-0 grid place-items-center px-8 text-center">
              <div>
                ...
                <p className="mt-1 text-[12.5px] text-fg3">The engine pulls Three.js r147 from unpkg on first mount — check the network, or vendor it into <code className="font-mono">public/vendor/</code>.</p>
              </div>
            </div>
          ) : (
            <>
        ...
        <aside className="hidden w-[308px] shrink-0 overflow-y-auto border-l border-line p-4 lg:block">
          ...
          {/* Live PLC image — tags and terminals from the Belegungsplan
              (536634 manual, p. 5). Green = asserted this phase. */}
          <div className="mt-5">
            <div className="flex items-baseline justify-between">
              <p className="font-mono text-[9px] uppercase tracking-[0.13em] text-fg3">Prozessabbild</p>
              <p className="font-mono text-[9px] text-fg3">{onCount}/{io.length} aktiv</p>
            </div>
            <div className="mt-2 max-h-[210px] overflow-y-auto rounded-[12px] border border-line bg-canvas/60 p-1.5">
              {io.map((pt) => (
                <div
                  key={pt.tag}
                  title={`Klemme ${pt.terminal} · ${pt.label}`}
                  className="flex items-center gap-2 rounded-[7px] px-1.5 py-[3px]"
                  style={{ background: pt.on ? 'color-mix(in oklab,#22c55e 12%,transparent)' : 'transparent' }}
                >
                  <span
                    className="h-1.5 w-1.5 flex-none rounded-full"
                    style={{ background: pt.on ? '#4ade80' : 'rgba(255,255,255,.13)' }}
                  />
                  <span
                    className="w-[30px] flex-none font-mono text-[10px]"
                    style={{ color: pt.kind === 'output' ? PRIMARY : 'var(--fg3)' }}
                  >
                    {pt.tag}
                  </span>
                  <span className="truncate font-mono text-[9.5px] text-fg3">{pt.label}</span>
                </div>
              ))}
            </div>
          </div>

          <div className="mt-5 rounded-[12px] border border-line bg-white/[0.02] p-3">
            <p className="font-mono text-[9px] uppercase tracking-[0.13em] text-fg3">Antriebe</p>
            <dl className="mt-2 space-y-1">
              {[
                ['Encodermotor', `${ACTUATOR_SPECS.encoderMotor.rpm} min⁻¹ · ${ACTUATOR_SPECS.encoderMotor.gearRatio}`],
                ['Impulse/U', `${ACTUATOR_SPECS.encoderMotor.pulsesPerOutputRev} (Abtrieb)`],
                ['S-Motor', ACTUATOR_SPECS.sMotor.gearRatio],
                ['Kompressor', `${ACTUATOR_SPECS.compressor.barGauge} bar · ${ACTUATOR_SPECS.compressor.currentMa} mA`],
                ['Farbsensor', ACTUATOR_SPECS.colourSensor.output],
              ].map(([k, v]) => (
                <div key={k} className="flex items-baseline justify-between gap-3">
                  <dt className="font-mono text-[9px] uppercase tracking-[0.1em] text-fg3">{k}</dt>
                  <dd className="font-mono text-[10.5px] text-fg2">{v}</dd>
                </div>
              ))}
            </dl>
          </div>
          ...
          <div className="mt-5">
            ...
            <div className="mt-2 max-h-[190px] space-y-1.5 overflow-y-auto rounded-[12px] border border-line bg-[color-mix(in_oklab,var(--canvas)_88%,transparent)] p-2.5">
